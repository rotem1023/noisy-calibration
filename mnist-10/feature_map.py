...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Create feature map for ham1000')
    ...
    if swin22:
        ...
        model_type = '22k'
        default_cfg = default_cfgs['swin_base_patch4_window7_224_in22k']
    else:
        model = SwinBase(in_timm_1k=True)
        model_type = '1k'
        default_cfg = default_cfgs['swin_base_patch4_window7_224']
    input_mean = default_cfg['mean']
    input_std = default_cfg['std']
    
    model = model.to(device)
    model.eval()

    _,_,validloader, testloader = get_loaders(batch_size=args.batch_size, norm_mean=input_mean, norm_std=input_std)

...
def train(model, combined_loader, device, train_loader, valid_loader, test_loader, accuracy_threshold):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, targets) in enumerate(combined_loader):
            ...
            print(f"Ecpoh: {epoch+1}/{num_epochs}, Batch: {batch_idx+1}, Loss: {loss.item():.4f}, "
...
if __name__ == '__main__':
    ...
    train_loader,train_loader_at_eval, val_loader, test_loader = get_loaders_ham(batch_size=args.batch_size)
    ...
    train(model=model, combined_loader=combined_loader, device=device,
          train_loader=train_loader, valid_loader=val_loader, test_loader=test_loader, accuracy_threshold=accuracy_threshold)
    
    train_predictions, tarin_labels, train_accuracy = predict_and_evaluate(model=model, loader=train_loader_at_eval, device=device)
    valid_predictions, valid_labels, valid_accuracy = predict_and_evaluate(model=model, loader=val_loader, device=device)
    test_predictions, test_labels, test_accuracy = predict_and_evaluate(model=model, loader=test_loader, device=device)
    
    os.makedirs("outputs", exist_ok=True)
    np.save(f"outputs/noisy_labels_train_dataset_{dataset}_acc{train_accuracy}", train_predictions.numpy())
    np.save(f"outputs/noisy_labels_valid_dataset_{dataset}_acc{valid_accuracy}", valid_predictions.numpy())
    np.save(f"outputs/noisy_labels_test_dataset_{dataset}_acc{test_accuracy}", test_predictions.numpy())
